...
filtered_d = filtered_d %>% 
  select(c("Subject", "Cond"), 
         contains("Game"), 
         -contains("Intro"), -c("WhichGames", "GameComments"), 
         starts_with("DinerDashWith"), c("SOFMusicEnemies", "SOFNoMusicEnemies")) 
```
...
```{r}
tiny_demo_d %>% pivot_longer(cols=-c("Subject", "Cond"), 
...
performance_hyp_long_d = performance_hyp_long_d %>% 
  mutate(ConfrontationalGame = grepl("SOF", Measurement), 
